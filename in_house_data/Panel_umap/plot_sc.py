...
os.makedirs('./plot_sc_umap', exist_ok=True)

models = ['pca', 'linearscvi', 'scvi', 'cellblast', 'scgpt', 'scimilarity', 'uce', 'cellplm', 'geneformer', 'scfoundation_new', 'scmulan']
...
for m in models:
    print(m)
    adata = anndata.read_h5ad("../shared_data/combined_sc_in_house.h5ad")
    embeds = np.load("../shared_data/umap_coords_sc/" + m + ".npy")
    index_df = pd.read_csv("../shared_data/sc_reference/" + m + "/cell_type.csv")
    ...
    plot(adata_new, "celltype", "./plot_sc_umap/umap_real_" + m + "celltype.pdf")
    plot(adata_new, "pred_cell_type", "./plot_sc_umap/umap_pred_" + m + "_celltype.pdf")
