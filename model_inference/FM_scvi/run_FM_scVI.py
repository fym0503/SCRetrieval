...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--lr", dest="lr", type=float, default=1e-3,
        help="Learning rate"
    )
    parser.add_argument(
...
def main(args: argparse.Namespace) -> None:
    ...
    adata = anndata.read_h5ad(args.input_rna)
    ...
    rna_emb = np.load(args.rna_pre)

    adata.obsm['X_FM'] = rna_emb

    print("[4/5] Training scVI...")
    adata_train = adata[adata.obs['tech']!='smartseq2'].copy()
    adata_test = adata[adata.obs['tech']=='smartseq2'].copy()
    PreSCVI.setup_anndata(adata_train, fm_key = "X_FM")

    vae = PreSCVI(adata_train, n_layers=args.n_layers, n_latent=args.n_latent, gene_likelihood=args.gene_likelihood)
    vae.train()

    embs_train = vae.get_latent_representation()
    PreSCVI.setup_anndata(adata_test, fm_key = "X_FM")
    embs_test = vae.get_latent_representation(adata_test)
    elapsed_time = time.time() - start_time

    all = np.zeros([adata.shape[0],embs_test.shape[1]])
    adata.obsm['X_scVI'] = all
    adata.obsm["X_scVI"][adata.obs['tech']!='smartseq2',:] = embs_train
    adata.obsm["X_scVI"][adata.obs['tech']=='smartseq2',:] = embs_test
    embs = adata.obsm['X_scVI'].copy()

    print("[5/5] Saving results...")
    args.output_path.mkdir(parents=True, exist_ok=True)
    np.save(args.output_path / 'embeds_scgpt_scvi.npy', embs)
    with open(args.output_path / 'run_info.yaml','w') as f:
        yaml.dump({
            "cmd": " ".join(sys.argv),
            "args": vars(args),
            "time": elapsed_time,
            "n_cells": adata_train.shape[0]
        }, f)
    ...
    fig = sc.pl.umap(
        adata,
        color=["celltype","map","tech"],
        frameon=False,
        return_fig=True,
        show=False,
    )
    fig.savefig(
                args.output_path /'embeddings_umap.png', dpi=300, bbox_inches="tight"
            )
